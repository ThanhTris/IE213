...
function GuestPage({ onChangeView, isAuthenticated }) {
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/warranties/${encodeURIComponent(tokenId)}`);
        ...
        <div className="search-bar-wrap">
          <div className="search-bar">
            ...
            <div className="search-actions">
              <button type="button" className="icon-btn" title="Scan QR" aria-label="Scan QR code" disabled>
          ...
          <p className="guest-search-hint">Or scan QR code from product packaging.</p>
          {!isAuthenticated && (
          ...
          {error && (
            <p className="guest-search-hint" style={{ color: "#b42318", fontWeight: 700 }}>
              {error}
            </p>
          )}
        </div>
          ...
          {result ? (
            <div className="result-top">
              <div className="result-meta">
                <span className="warranty-badge">{result.status ? "Active Warranty" : "Inactive Warranty"}</span>
                <h2 className="device-name">{result.productInfo?.productName || "Warranty record"}</h2>
                <p className="serial-muted">Serial - {result.serialNumber}</p>
                <span className="token-id">Token ID - {result.tokenId}</span>
                <p className="serial-muted">Expiry - {formatExpiry(result.expiryDate)}</p>
              </div>
            </div>
          ) : (
            <div className="result-top">
